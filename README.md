# 📦 UBT Lost & Found – Website për Humbje dhe Gjetje Sendesh

Ky projekt është një **faqe interneti për të gjetur gjërat e humbura në kampus**, e ndërtuar si pjesë e një detyre grupi për universitetin. Qëllimi i faqes është të ndihmojë studentët dhe stafin e universitetit të publikojnë njoftime për sende të humbura ose të gjetura, në mënyrë që pronarët e tyre t’i gjejnë më lehtë.

---

## 👥 Anëtarët e Grupit

- Shend Tytynxhiu - ID: 242573278, st73278@ubt-uni.net
- Ekloan Kalludra - ID: 242573584, ek73584@ubt-uni.net
- Andre Rasi - ID: 242573800, email: ar73800@ubt-uni.net

---

## ⚙️ Teknologjitë e Përdorura

Projekti është ndërtuar duke përdorur vetëm teknologjitë bazike të zhvillimit të faqeve web:

- **HTML5** – për strukturën e faqes
- **CSS3** – për stilizim dhe dizajn responsive
- **JavaScript (Vanilla JS)** – për funksionalitet dinamik në anën e klientit
- **PHP** – për funksione bazike në anën e serverit (si ruajtja e të dhënave ose ngarkimi i fotove)

---

## 🔎 Pershkrim i Kodit

Ne cdo `folder` gjendet njw `README.md`, ku shpjegohet struktura e `file-ave` mw detajisht.

---

## ⚙️ Funksionalitetet Kryesore

- Publikimi i njoftimeve për sende të **humbura** ose **të gjetura**
- Kërkimi dhe filtrimi i njoftimeve sipas **kategorisë** ose **datës**
- Mundësia për të **ngarkuar imazhe** të sendeve
- Dizajn **responsiv** që përshtatet në pajisje të ndryshme
- Ndërfaqe e thjeshtë dhe e lehtë për t’u përdorur

---

## 🚀 Si të Niset Projekti Lokalisht

1. Shkarko ose klono projektin nga GitHub:
   ```bash
   git clone https://github.com/Shend-T/Lost_And_Found.git
   ```
2. Hap 'folder-in' e projektit:
3. Hap 'file-in' `php/index.php` ne 'browser-in' tuaj përmes një serveri lokal (p.sh. XAMPP).

---
